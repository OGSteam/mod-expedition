...
if (class_exists("Callback")) {
    class eXpedition_Callback extends Callback
    {
        ...
        public function eXpedition_attack($rapport)
        {
            global $io, $user_data;
            logging('ATTACK_RC');
            if (attack_analysis($user_data['user_id'], $rapport)) return Io::SUCCESS;
            else
                return Io::ERROR;
        }
...
function eXpedition_analysis($uid, $galaxy, $system, $timestamp, $content)
{
    ...
    if ($db->sql_numrows($db->sql_query($query)) == 0) {
        ...
        // RESSOURCES ?
        elseif (preg_match($regexRess, $content, $expRess) != 0) {
            logging("RESSOURCES");
        ...
        // MERCHANT ?
        elseif ((preg_match($regexMerch[0], $content) != 0) || (preg_match($regexMerch[1], $content) != 0)) {
            logging("Marchand");
            $query =
                "INSERT INTO " . TABLE_EXPEDITION . "
                (user_id, date, pos_galaxie, pos_sys, type)
        ...
        // Items
        elseif (preg_match($regexItems, $content, $expItems) != 0) {
            logging("Items");
            $itemType = $expItems[1];
